...
import BGCoverNamePlanMax from '../../../assets/layout/bg-cover-name-plan-max2.svg';
...
export default function CardPlanIdea2(params: CardPlanProps) {
	...
	return (
		<div className="flex max-h-full min-h-full w-full max-w-lg flex-col max-md:justify-center lg:w-[33%]">
			{/* bg-gradient-to-b from-pink-300 to-purple-300 p-px */}
			<div className="flex min-h-full w-full flex-col rounded-3xl  ">
				<Card
					className={cn(
						'flex min-h-full w-full transform flex-col rounded-[calc(1.5rem-1px)] bg-white font-kanit transition duration-700 hover:scale-105',
						params.typePlan == 'max' && 'bg-cm-primary/90',
					)}
				>
					<CardHeader className="">
						...
						<CardDescription>
							<div className="flex w-full flex-col pt-10 font-kanit">
								<span
									className={cn(
										'relative mt-6 self-center text-6xl font-semibold uppercase leading-[48.24px] text-purple-800 max-md:text-6xl max-sm:text-5xl',
										params.typePlan == 'max' && 'text-white',
									)}
								>
									¥{params.valuePlan}
								</span>
								<span
									className={cn(
										'relative mt-1.5 w-full px-10 text-end text-xl font-medium uppercase text-purple-800 max-md:mr-20',
										params.typePlan == 'max' && 'text-white',
									)}
								>
					...
					<CardContent className="flex h-full w-full flex-col items-start justify-start gap-1 px-2.5 ">
						<span
							className={cn(
								'text-md font-nunito font-medium max-md:text-sm',
								params.typePlan == 'max' && 'text-white',
							)}
						>
							What's included:
						...
						<div className="flex flex-col items-start justify-start px-2">
							{params.itemsIncluded.map((item, index) => (
								<div
									key={index}
									className={cn(
										'relative flex  items-center justify-start gap-2 py-2 pl-1 text-gray-800 max-md:pl-5',
										params.typePlan == 'max' && 'text-white',
									)}
								>
